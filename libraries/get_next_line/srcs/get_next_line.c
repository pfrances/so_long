...
/*   Updated: 2022/11/28 11:43:56 by pfrances         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	search_line_break(char **result, char **save, size_t *i)
{
	while ((*result)[*i] != '\0')
	{
		if ((*result)[*i] == '\n')
		{
			*save = ft_strndup(*result + *i + 1,
					ft_strlen_gnl(*result + *i + 1));
			if (!*save)
				return (ERROR);
			ft_bzero_gnl(&(*result)[*i + 1], ft_strlen_gnl(&(*result)[*i + 1]));
			return (SUCCESS);
		}
		(*i)++;
	}
...
static char	*get_line_and_save(char **result, char **save, char **buff, int fd)
{
	...
	while (1)
	{
		ft_bzero_gnl(*buff, ((size_t)GNL_BUFFER_SIZE + 1));
